  ...
  async callback(ctx) {
    ...
    if (provider && provider !== 'local') {
      ...
      try {
        ...
        if (provider === 'apple') {
          const acceptHeader = String(ctx.request.headers?.accept || '').toLowerCase();
          const requestedJson =
            acceptHeader.includes('application/json') ||
            acceptHeader.includes('application/vnd.api+json') ||
            acceptHeader.includes('application/*+json');
          const queryResponseFormat =
            typeof ctx.query?.response === 'string'
              ? ctx.query.response.toLowerCase()
              : undefined;
          const enforceJsonResponse = queryResponseFormat === 'json';

          if (!requestedJson && !enforceJsonResponse) {
            try {
              const appleSettings = (await strapi
                .plugin('strapi-plugin-apple-provider')
                .service('service')
                .getSettings()) as { redirectUrl: string | null | undefined };

              const targetRedirect = appleSettings?.redirectUrl ?? '';

              if (targetRedirect) {
                const codeFromRequest =
                  (typeof ctx.request.body?.code === 'string' && ctx.request.body.code) ||
                  (typeof ctx.query?.code === 'string' && ctx.query.code) ||
                  undefined;
                const stateFromRequest =
                  (typeof ctx.request.body?.state === 'string' && ctx.request.body.state) ||
                  (typeof ctx.query?.state === 'string' && ctx.query.state) ||
                  undefined;

                const destinationUrl = new URL(targetRedirect);
                const searchParams = destinationUrl.searchParams;

                if (codeFromRequest && !searchParams.has('code')) {
                  searchParams.set('code', codeFromRequest);
                }

                if (stateFromRequest && !searchParams.has('state')) {
                  searchParams.set('state', stateFromRequest);
                }

                destinationUrl.search = searchParams.toString();

                const existingHash = destinationUrl.hash.replace(/^#/, '');
                const hashParams = new URLSearchParams(existingHash);
                hashParams.set('provider', provider);
                hashParams.set('jwt', result.jwt);
                if (result.refreshToken) {
                  hashParams.set('refreshToken', result.refreshToken);
                }

                if (!hashParams.has('status')) {
                  hashParams.set('status', 'success');
                }
              ...
              strapi.log.warn(
                `[users-permissions] Apple redirect fallback failed: ${
